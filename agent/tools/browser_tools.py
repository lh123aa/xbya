...
def extract_snippets(html: str, limit: int = 5) -> List[Dict[str, str]]:
    ...
    提取顺序（**每一层都是为了绕开实测出来的坑**）：

    1. 只在**结果块**里找（Bing 的 `<li class="b_algo">`）—— 最干净
    2. 退回 `<h2>`/`<h3>` 里的链接 —— 通用一些，但仍会命中页脚推荐位
    3. 最后才是任意链接 —— 纯兜底

    这个顺序不是优化而是必须：实测 Bing 结果页里
    **页面最前面的 `<a>` 是导航栏**（Web / 图片 / 视频 / 地图 / 资讯 / 航班），
    一上来就通用扫描的话，任何查询都只会得到"搜到 5 条：图片、视频"这种胡说，
    而真正的 10 条结果一条都拿不到；只用 h2/h3 也还不够 ——
    中文查询「上海天气」命中的是页脚论坛推荐位（一堆蓝屏/OneDrive 帖子），
    同样答非所问。**这两个坑都只有真的发一次网络请求才会暴露**
    （拿假 HTML 写的用例一条都测不出来）。
    """
    text_html = html or ""
    results: List[Dict[str, str]] = []
