...
class TestRouterExitBranches:
    ...
    def test_list_no_dir(self, r):
        """列目录无别名 → **不许凭空造默认目录**（D46 修正）。

        ⚠️ 这条用例原先断言的是 `{"dirs": ["Desktop"]}`，注释写着"默认桌面"。
        那是**把缺陷固化成预期**：于是"没说目录"被伪装成"说了 Desktop"，
        导致纯闲聊被高置信度判成文件操作 —— 用户从没让它操作任何目录，
        却收到"哎呀，刚才那个「Desktop」我没太听明白呢"（实测日志）。

        正确行为：没说就不填。让上层按"缺目标"降置信度、回落闲聊；
        真需要默认目录时，由工具层在**用户确实表达了文件意图**之后兜底。
        判据：**默认值只能补"已确认的意图"，不能制造意图**。
        """
        assert r._extract_list_params("看看有什么", "看看有什么", {}) == {}
