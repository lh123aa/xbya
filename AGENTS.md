...
>
> | **D32 字幕动态更新（整段闪现后被逐句擦成碎片／控制标记漏屏／截断无提示）** | ✅ **已修**（用户反馈"字幕动态更新没有做好"）。① 播放线程**逐句覆盖且不带上一句** ⇒ 整段闪现后被切碎（实测 `@0.75s 整段 → @1.56s 第1句 → @2.96s 第2句`）；② `_sanitize_text` 只清 markdown、**不清 `[silent]`/`[thinking]` 等内部控制标记**，脏字直接上屏；③ `_wrap_text` 达最大行数时 `_clamp_line(…, ellipsis=True)` 在"该行本已放得下"时**原样返回不加 …** ⇒ 超出部分被**静默丢弃且无提示**。修复：改为**滚动窗口**（当前句 + 上一句，实测 `@1.70s 第1句 → @3.02s 第1句+第2句 → @4.31s 第2句+第3句`），保留"先整段上屏"（静音/TTS 失败时不白屏，既有契约）；清洗新增控制标记规则（普通方括号/markdown 链接不受影响）；截断显式补 `…` 且不超宽。**另删一处死代码**：循环末尾截断分支结构上不可达（穷举 55986 个文本 + 4000 长样本，命中 **0** 次）。`tests/test_subtitle_dynamic.py` 18 条 + **逐条反方向验证**。⚠️ 中途我改成"完全不预写全文"被既有用例抓出**真实回归**（无声/降级路径正文永不显示），已改回两全方案。明细见 `docs/agent/sprite-history.md` |
>
> **D27~D32 的完整明细**（逐轮过程、实测数字、误判判据、定位手法）
