...
> 它们是**已登记的清单项**（B1/B2 有代码可改、B3~B5 从未实测），
> 所以**算闭环缺口，是我欠的活儿**，不是"可选增强"。**必须做完才算闭环**。
> 账本上它们的证据走 `planned_evidence`（G13 不查），落档后要挪进 `evidence`——
> 这个动作就是"完成"的可检查定义。
...
> | **D35 TTS 完全没声音（"推荐音色"根本不存在）** | ✅ **已修**。设置界面「晓伊（**欣雅推荐**）」写的是 `zh-CN-xbyaNeural` —— 把项目名塞进 `zh-CN-<name>Neural` 模板**拼出来的假名字**。危险在于**不报错**：Edge 对未知音色只回**空音频流**，界面能选中能保存、用户**一个字都听不到**（实测 14 次调用 14 次失败）。它是**菜单推荐项+设置 fallback+插件默认值**三处默认值。修复：改名 `zh-CN-XiaoyiNeural` + **空音频判失败**守卫。真机 TTS **0→14 成功**。明细见 `sprite-history.md` |
> | **D36~D43 语音管线端到端监督分析** | ✅ **已修 4 项 + 2 项确认非缺陷**。**ASR→对话 通过率仅 11%**（9 条识别结果 8 条静默消失）。① D40 静默丢弃补日志；② D41 备用模型是推理型（思维链 500~800 字，8.76s）→ 换 mini 变体 **13.24s→3.66s**；③ D42 配置指向**近乎死流**设备 → 改 idx=1，底噪 4346→**92**、阈值 27012→**216**；④ D43 增益补偿放在阈值判断之后等于没放 → 提前，触发 **0→15 次**。⚠️ **最终根因是麦克风信噪比**：满量程仅 **1.66%** ⇒ 系统输入增益过低；**等比增益改变不了峰谷比** ⇒ 需用户调系统音量/换近距离麦。**判据：能打开≠有信号，判活性唯一标准是"数据在变"。** 明细见 `sprite-history.md` |
>
> | **D49~D50 配置说谎 + 插件加载依赖调用顺序** | ✅ **已修**（系统性功能完整性审计查出）。**D49**：`plugins.embedding/vector_db.engine` 写的 `embed_anything`/`leann` 两个目录**只有 0 字节 `__init__.py`、没有 `plugin.py`** ⇒ `scan()` 整个跳过；且 `file_service.py:406` 用 `is None` 判降级，而加载失败给的是**空对象不是 None** ⇒ 降级**不可达**。⚠️ 严重性**低**（该子系统生产调用者 **0 个**）但**配置在说谎**。**D50**（更值得写）：`get_plugin_loader()` 单例的**"谁负责 scan()"没有归属** ⇒ 受控 A/B：先 scan → `WatchfilesMonitor`；独立构造 → **`NullFileMonitor`**；报错「插件不存在: X」**那句话是错的**。修：配置显式 `null` + 懒扫盘 + 报错自证。反方向 **5/5 变红**。明细见 `sprite-history.md` |
>
> **D27~D50 的完整明细**（逐轮过程、实测数字、误判判据）见 **`docs/agent/sprite-history.md`**
