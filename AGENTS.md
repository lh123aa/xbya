...
│
├── tools/                           ← 验收与运维脚本
│   ├── run_acceptance.py            # 一次跑完 12 关（G1~G12）+ 逐关留原始输出
...
│   ├── verify_f7_real_services.py   # F7：真实翻译/天气/提醒（27 项）
...
| D13 | **提醒不持久化** | `ReminderTool._items` 只在内存里；`agents/tracker_store.py` 已有的原子落盘套路没有复用 | 进程重启后**未到点的提醒会丢**（用户以为还等着，其实没了）。已设的提醒在本次进程内有效（F7 已让它真的响），跨重启无效 | 复用 `tracker_store.py` 的原子写 + 启动恢复，把提醒落 `data/agent_reminders.json`。**本轮未做，如实登记**（见 §17.4） |
| D14 | **两个不兼容的 EventBus 并存** | `core/event_bus.py`（枚举 + `subscribe/emit(EventType, dict)`）与 `core/kernel/events.py`（字符串 + `on/emit(str, **kwargs)`）同名不同契约 | 极易再次接错 —— 本轮修掉的第一个缺陷就是"把前者传给了只认后者的 Agent 层"，代价是 Agent 层**自 P0 起在真实应用里全程静默降级**，而所有脚本级验收都测不出来 | 短期：两处 docstring 写明差异（`core/app.py::_setup_agent_layer` 已有对照表）。长期：把 Agent 层的事件总线统一到内核实现，或给 `core.event_bus` 加适配层。**本轮不做**（会牵动现有语音管线，违反"向后兼容"原则） |
| D15 | **仓库无版本控制** | 工作区的 `.git` 是指向 `E:/程序/桌面宠物/xiaoyi-desktop-pet/.git/worktrees/...` 的**指针文件**，而该父仓库目录**已不存在** → `git status` 直接 fatal | ~41k 行代码（110 源文件 + 54 测试文件）**没有任何回溯能力**：改坏一行无法 `git checkout`，不可 diff、不可 bisect、不可协作。比任何功能缺口都危险 | **P4-A1**（P4 的第一件事，见 `docs/agent/p4-plan.md`） |
...
| 编号 | 项 | 一句话理由 |
|------|----|-----------|
| P4-A1 | 重建版本控制（D15） | 没有它，后面所有改动都不可回滚 |
| P4-A2 | 提醒持久化（D13） | 全项目唯一"对用户撒谎"的功能缺口 |
| P4-A3 | 人工验收执行包 | 麦克风 / GUI 动效 / 人耳听感零验证，且只能人做 |
...
## 验收阶段（`tools/run_acceptance.py` 一次跑完 12 关）

| 关卡 | 实测（2026-09-10 **终轮**） | 结论 |
|------|------|------|
| G1 语法检查 | 全部改动模块 `py_compile` 通过 | ✅ |
| G2 全量回归 | **2463 passed / 1 skipped / 0 failed** | ✅ |
| G3 覆盖率（项目口径） | **6952 语句 / 0 未覆盖 / 100%** | ✅ |
| G4 P1 端到端 | **18/18** | ✅ |
| G5 P3 端到端 | **44/44** | ✅ |
| G6 指标测量 | **8/8 PASS**（M1 路由准确率 100%、M2 P95 0.050ms、M3 2.0ms、M4 10.5ms、M5 拒绝 8/8、M6 走回收站） | ✅ |
| G7 G0 示例可运行 | **113/113** | ✅ |
| G8 F3 真实 LLM | **16/16 通过、0 失败、2 跳过**（配额 429 → 跳过） | ⚠️ 外部依赖，**有 2 项本轮没验到** |
| G9 F4/F5 真实网络 + 真机调用 | **35/35** | ✅ |
| G10 F7 真实生产力服务 | **27/27** | ✅ |
| G11 真实 GUI 启动与退出 | **16/16**（退出码 0） | ✅ |
| G12 语音 5 场景回环 | **50/50（失败 0、降级 0）** | ✅（**固定测激**下，见下） |

> **内部关卡 6/6 通过**（G1~G7）。外部依赖关卡 6/6 退出码 0，但 **G8 有 2 项被跳过**：
> 免费档 8000 TPM 配额被打满，模型没被真正问到 → 记 `[SKIP]` 而不是 `[FAIL]`
> （缺陷 18 的处置），汇总报告会把有跳过项的关卡**单独标出来**，
> 避免"退出码 0"被读成"这轮验过了"。原始输出与汇总在 `docs/agent/evidence/acceptance/`。
>
> ⚠️ **G12 绿了也不等于"语音验收完成"**：它默认吃**固定测激**（TTS 替身的那一版音频固定），
> 测的是**管线**；不覆盖麦克风采集、GUI 动效、人耳听感；§7.3 的"截图留证"仍需人做。
...
**未修 / 待下轮**（详见 `docs/agent/f-closure.md` §17.4）：
收列表参数的宽松转换、429 配额如何在 `/status` 里可见、提醒持久化（D13）、
两个 EventBus 并存（D14）、麦克风硬件采集、GUI 动效与人耳听感、
短句确认语在 ASR 下不稳（「算了」2/4）、TTS 替身偶发劣化（成因未完全定位，已隔离）。
~~繁简归一化导致删除丢目标~~ → **已修**（缺陷 14 + 22，G12 场景 2 现为 ✅）。
