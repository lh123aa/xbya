...
| 测试用例 | 全量 `pytest tests` | **2871 passed / 1 skipped / 0 failed**（另有 1 项 `test_run_injects_app_and_loads_pet` 在**欣雅正在运行**时必红 —— 单实例锁按设计拦下 `app.run()`，已实测确认该失败在 HEAD 上完全相同，属环境互斥而非回归；关掉运行中的实例后通过）。2779 → 2871 = D27~D31 新增 | skipped 的那 1 项是环境相关，不是"忽略失败" |
