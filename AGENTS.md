...
| 测试用例 | 全量 `pytest tests` | **2891 passed / 1 skipped / 0 failed**（`test_run_injects_app_and_loads_pet` 在**欣雅正在运行**时必红 —— 单实例锁按设计拦下 `app.run()`，已用 `git stash` 实测确认该失败在 HEAD 上完全相同，属环境互斥而非回归；实例关闭后即通过）。2779 → 2891 = D27~D32 新增 | skipped 的那 1 项是环境相关，不是"忽略失败" |
...
> | **D27 抠图边缘毛刺 / 裙子镂空 / 臂腰间隙被填满** | ✅ **已修**。① 硬二值化 `np.where(alpha>128,255,0)` 把抗锯齿压成 0/255 → 毛刺；② **我一度误判源图是"无 alpha 的白底图"并重算 alpha**，反而打穿白裙子、填满臂腰镂空。源图**本就是带 alpha 的 PNG**（256 级 / 58% 透明 / 1.39% 半透明）。改用 `cutout_source_alpha()` + `keyer: "alpha"`（**有现成 alpha 就用，绝不重算**） |
> | **D28 眨眼：位置不准，且合成路线走不通** | 🟡 **合成路线已实测否决，待人工出图**。整只眼仅 **13×11 = 143 px**（虹膜 61 px），**7 种合成做法全部失败**。改为 `blink_by_blend()`：**有闭眼素材就逐像素混合，无素材则不眨眼**（宁可不眨也不贴假的）。`_load_closed()` + `src_closed` 字段已备好，**只差一张闭眼立绘** |
> | **D29 点头幅度过大 + 两个动画是"死"的** | ✅ **已修**。真正可见的是**纵向位移**（`nod` 旋转只有 0.088°、根本不可见）；`listen`/`sleep` 是唯一的 4.00px，`sleep`/`sad` 位移**恒为正**、整像素取整后每帧相同 = **死动画**。修后八种动画全在 1~2px 且**都有 ≥2 个位移档位**。规则：**逐帧摆动必须围绕 0** |
> | **D30 字幕不显示（跨线程投递被静默丢弃）** | ✅ **已修**。`_invoke_on_main` 用 `QTimer.singleShot(0, fn)` 跨线程投递 —— Qt 在**调用线程**建定时器，工作线程无事件循环，**回调被静默丢弃**（受控实验：主线程 1 次 ✓ / 后台线程 0 次 ✗）。真实链路全在后台线程调 `show_bubble()` ⇒ 字幕永不显示而日志正常。改用 `QMetaObject.invokeMethod(…, Qt.QueuedConnection)`。**新增 2 条用例 + 反方向验证**（塞回旧实现 → 2 failed） |
> | **D31 位置记忆写进"屏幕外坐标"＋ VRM 模式整段跳过恢复** | ✅ **已修**（用户反馈"每次启动位置是上次关闭的位置，有时重启位置就跑到屏幕左上角了"）。三处独立缺陷：① `_edge_snap()` 先把窗口移到屏幕外（右边只剩 8px 手柄），`mouseReleaseEvent` 紧接着 `_save_position()` **无条件**存下这个坐标（**实测存进 `(1911,300)`**）；② `_restore_position` 判据**单向** —— 只挡 `x >= right-10` / `y >= bottom-10`，**没有**左边/上边检查；③ `load_pet` 在 `render_mode=="vrm"` 时**提前 return**，恢复整段跳过、窗口停在 `__init__` 的 `(0,0)` —— 正是"左上角"。修复：`_pre_snap_pos` 记吸附前位置供存档用；新增**对称**判据 `_is_position_usable()`（按**所有屏幕**算可见交集，要求 ≥60px）供恢复/保存共用；VRM 分支补上恢复；另加两道闸（恢复完成前不写、贴边态不写屏幕外坐标）。**端到端实测**（真启动应用）：合法 `(760,366)`→原样恢复；屏幕外 `(-900,100)`→落到可见的 `(1631,120)`；贴边后重启→恢复到拖拽处 `(1687,300)`（旧代码存的是 `(1911,300)`）。`tests/test_window_position.py` 11 条用例 + **逐条反方向验证** |
>
> | **D32 字幕动态更新（整段闪现后被逐句擦成碎片／控制标记漏屏／截断无提示）** | ✅ **已修**。滚动窗口（当前句+上一句）、清洗 `[silent]`/`[thinking]` 控制标记、截断显式补 `…`；另删一处**穷举证明不可达**的死代码。明细见 `docs/agent/sprite-history.md` |
> | **D33 麦克风关了还能听到声音（开关只改内存 + 闸门缺失）** | ✅ **已修**（用户报告"🎤 麦克风关了，她还能听见我说话并回应"）。**两处独立缺陷**：① `_toggle_mic(False)` **只设内存 `_monitor_enabled`，不写 `config.voice.listening`** —— 此后任一次「保存设置」（`apply_settings` 按 `voice.listening` **无条件重启监听**）或**下一次启动**都会把麦克风重开，而菜单仍显示"已关闭"；② `_on_speech_captured` **没有 `_monitor_enabled` 闸门** —— 只要底层流因任何原因还活着，捕获到的语音就被正常处理并回应。修复：开关动作**落盘** + 捕获入口加**用户意图闸门** + 热键与菜单**收敛到同一条路径**。`tests/test_mic_toggle_persist.py` 9 条 + **逐条反方向验证**。⚠️ 我一度误判为"关麦后仍采集 ~6 秒"，实为**测量假象**（在监听尚未进入读循环时关闭，测的是"没开始读"）—— 修正后实测关闭延迟 **~40ms**。明细见 `docs/agent/sprite-history.md` |
>
> **D27~D33 的完整明细**（逐轮过程、实测数字、误判判据、定位手法）
> 见 **`docs/agent/sprite-history.md`** —— 与 §11~§17 移到 `phases.md` /
> `p3-history.md` / `f-closure.md` 同一处理方式（**原文一字未删，只是换位置**）。
> 移出去的原因：本文件有 **65536 字节**硬预算，超了会被**静默截断**
> （本轮实测：写入后 67222 字节、被截到 65206 —— 权威文档被悄悄截尾，
> 比内容长本身危险得多）。
>
...
> "头部点头的幅度调小一些""这个版本不显示字幕了"）。
> 这四项我**各修错过一次**：D27 误判成白底图、重算 alpha 反而打穿裙子；
...
> · **Qt 跨线程投递不能用 `QTimer.singleShot`**（会在调用线程建表、回调丢弃）；
>   且**"从外面测都对"时要进进程内打点** —— 外部观测会被分层窗口、截图失真等假象骗过。
