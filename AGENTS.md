...
| P5-B1~B5 | D16 守护 / voice_service 传参 / ST+vec0 / medium 长句 / SERP fixture | ⏳ 未到终态（P1 可选增强，非已登记缺陷） | 计划路径见 `tasks-p5.json` 的 `planned_evidence` |
| P5-C1~C2 | 轮换 key / 人工验收 M1~M5 | 🟡 待人工 | 见 §六 末"能力边界" |
| P5-C3~C8 | 6 项明确不做（D3 / 天气 IP / D7 / 繁简边界 / 诱饵页 / 历史账本） | ⬜ 已登记 | 各附可反驳理由 |

> **"未到终态"也是终态的一种说法，但它必须写明是什么**：P5-B1~B5 是**可选增强**，
> 不是已登记缺陷 —— 做 3 项和做 5 项都满足"闭环"，所以它们**不构成闭环缺口**，
> 也不会被写成"已完成"。`tasks-p5.json` 里它们的证据走 `planned_evidence`
> （G13 不查），一旦落档就要挪进 `evidence` —— 这个动作就是"完成"的可检查定义。
