...
> | **D27 抠图边缘毛刺 + 裙子镂空 + 臂腰间隙被填满** | ✅ **已修（换新立绘那一轮，含一次自我纠正）**。**真正的根因有两层**：① `build_base()` 结尾的 `np.where(alpha > 128, 255, 0)` 把 LANCZOS 抗锯齿压成 0/255 → 128px 精灵上的阶梯毛刺（旧底图实测 **alpha 唯一值 = 2、半透明像素 = 0**）。② **我一度误判源图是"无 alpha 的白底图"**，于是用白底色键 + 边界泛洪**重算 alpha**，反而制造了两个新破坏：**白裙子被打穿**（裙子高光 ~245 距纯白只有 10，色键判成背景）与**胳膊与腰之间的镂空被填满**（那处背景被手臂和身体围住、不与画布外边界连通，泛洪不认它是背景）。用户指出"我已经是扣好图的 png 图，按 png 抠图结果为准"后核对：源图**本来就是带 alpha 的 PNG**（256 个 alpha 值 / 58.0% 全透明 / 40.6% 不透明 / **1.39% 半透明** —— 那 1.39% 正是画师做好的抗锯齿边）。**修法**：新增 `cutout_source_alpha()` 与 `keyer: "alpha"`（默认），**有现成 alpha 就用它，绝不重算**；`white`/`rembg` 降为"源图确实没有 alpha 时"的备用路径。**教训**：抠图方式应由"源图有没有 alpha"决定，不是由"背景看起来是什么颜色"决定。**验收**：①裙子中心带 x[51..76] y[128..257] **最小 alpha = 255、不透明率 100%**；②臂腰镂空左侧连续 242 行 / 右侧 240 行；③alpha 唯一值 **249**（抗锯齿在）。三个判据脚本化 |
> | **D28 眨眼：位置不准，且合成路线根本走不通** | 🟡 **终态：合成路线已实测否决，改为等待素材（待人工出图）**。**逐轮过程**：① **眨眼看不见**——源图眼框映射到画布后**框顶比虹膜上缘低 6px**，补丁只盖住下半。② **时长失控**——相位容差判定实测闭眼持续 **7~9 帧 ≈ 0.6~0.75 秒**（真人眨眼才 100~150ms）。③ **位置仍错位**——程序判据（亮度/颜色连通域）都被"虹膜高光+睫毛阴影"带偏，改用源图 3 倍放大 + 画框复核定稿。④ **闭眼线画成黑条**——原来画两行，13×11 的框上就是 2px 全宽黑条（逐行扫描 y=45 有 13 个暗像素）。⑤ **点头是固定频率**——我第一版用"无理数比 1.618/2.414"想让点头随机，**实测全面变差**（主频能量 66.6%→69.5%、首尾落差 0.069→0.165）：20~24 个采样点下这些频率会**混叠**回低频；正确做法是把**节拍表本身写乱**（`_nod_schedule`，3 个强度不等、位置不等距的点头），实测主频能量 **66.6%→40.0%**、首尾落差 **0.069→0.0052**。⑥ **关键结论：合成眨眼在 128px 下不可能做好**。实测整只眼睛只有 **13×11 = 143 px**（虹膜占 61 px）；要同时做到"盖住 61 个虹膜像素"+"边缘与周围皮肤无缝"，在这个像素预算下不可行。**7 种合成做法全部失败**（肤色椭圆补丁/纵向压扁/上方皮肤拉伸/上下眼皮对合/眼睑弧/宽羽化/双睑线），实测数据与复核图留档在 **`docs/agent/blink-solutions.md`** 与 `evidence/p5/_blink_methods*.png`。**当前处置**：`f_idle`/`f_listen` 改走 `blink_by_blend()`——**有 `src_closed` 素材就逐像素混合两张立绘（真实、无破绽）；没有素材就不眨眼**（宁可不眨，也不贴假的）。已备好 `_load_closed()` 与 profile 的 `src_closed` 字段，**只差一张闭眼立绘**。**待人工**：用在线 inpainting 把双眼重绘成闭眼，存 `resources/source/avatar_closed.webp`，在 profile 里填 `src_closed` 即可生效。**可判定判据**：闭眼帧眼区虹膜暗像素 = 0；闭眼帧与脸颊色差 < 5（现合成方案 14.1）；眼框四边色差跃变 < 3。其余部分（呼吸、不规则点头、9 个整帧表情）**已交付且可信**，`tests/test_animation.py + test_pet_window.py` **127 passed** |
>
> | **D29 点头幅度过大 + 两个动画其实是"死的"** | ✅ **已修**。用户反馈"头部点头的幅度调小一些"，量下去却查出**两个更严重的问题**。**① 真正可见的是纵向位移，不是旋转**——`f_listen` 的 `nod` 旋转量实测极差只有 **0.088°**（几乎不可见），而 `breath_y` 是 **4.00 px**。调幅度必须调后者；只调 `nod` 的系数完全没有效果（我第一版差点调错）。**② `listen`/`sleep` 是唯一的两个 4.00px**，其余 6 个动画都在 1.0~2.0px（idle 1.95 / think 1.60 / happy 1.71 / sad 1.20 / stare 1.60 / dance 1.04）——在 128px 宽的角色上，4px = **整个人上下跳 3%**，倾听是个持续状态，长时间挂着会显得"坐不住"。**③ 顺带查出两个动画是死的**：`_shift()` 是**整像素**的（`int(round(dy))`），而 `f_sleep` 原式 `1.2 - 2.0*sin` 值域 [−0.8, 3.2]、`f_sad` 原式 `1.8 + 0.6*sin` 值域 [1.2, 2.4] —— 两者**恒为正**，取整后**每一帧都是同一个值**，与静止参考帧做整数互相关实测 `dy 全为 0`，整段动画**僵硬不动**。**教训**：`transform()` 的位移是整数像素，任何"姿态偏移"若写成常量（每帧相同）就等于没有动画；**逐帧摆动必须围绕 0**。**修法**：`f_listen` 呼吸 ±2.0→±1.0、nod 0.095→0.075；`f_sleep` 改 `-1.0*sin`（围绕 0）、`f_sad` 改 `0.7*sin`（围绕 0）；`f_idle` 1.95→1.46px。**验收**（脚本化，用整数互相关测真实位移档位）：八种动画全部落在 **1~2px**、且**每个都有 ≥2 个位移档位**（不再是死的）——idle/listen/sleep/sad/stare/dance 为 `[−1,0]`、think 为 `[0,+1]`、happy 为 `[−2,−1,0]`。判据写法本身也可复用：**"位移档位只有 1 个"就是死动画**。`tests/test_animation.py + test_pet_window.py` **127 passed** |
>
> ⚠️ D19 / D20 都是**本轮执行中查出来的**，不在最初 18 项里 ——
> D21~D23 又是**再下一轮**查出来的（改角色那一轮）。D27/D28/D29 是**再再下一轮**
> （用户反馈"抠图边缘都是毛刺""优化眨眼动效""裙子不应该有透明通道"
> "胳膊和腰部中间也是镂空的""眨眼的位置还是错位的""头部点头的频率随机一些"
> "头部点头的幅度调小一些"）。
> 这三项我**各修错过一次**：D27 误判成白底图、重算 alpha 反而打穿裙子；
> D28 用"无理数比"想让点头变随机，实测指标全面变差；
> D29 一开始差点去调 `nod` 旋转系数（实测那只有 0.088°、根本不可见）。
> 登记它们的价值不在于"修好了"，而在于**把误判的判据和实测数字留下**：
> · 抠图方式该由"源图有没有 alpha"决定，不是由"背景看起来是什么颜色"决定；
> · 短循环动画要"不规律"，该把**节拍表写乱**，不是堆频率 ——
>   只有 20~24 个采样点时，无理数频率会被混叠回低频，越叠越像节拍器；
> · **逐帧摆动必须围绕 0**（位移是整像素，常量偏移 = 死动画）；
