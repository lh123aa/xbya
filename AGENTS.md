...
> | **D36~D43 语音管线端到端监督分析** | ✅ **已修 4 项 + 2 项确认非缺陷**。漏斗量化发现 **ASR→对话 通过率仅 11%**（9 条识别结果 8 条静默消失）。① D40 静默丢弃补日志；② D41 备用模型是推理型（思维链 500~800 字，平均 8.76s）→ 换 mini 变体 **13.24s→3.66s**；③ D42 配置指向**近乎死流**的设备（变化 5~9/18）→ 改 idx=1，底噪 4346→**92**、阈值 27012→**216**、死流恢复 16→**0 次**；④ D43 增益补偿位置错误（放在阈值判断之后等于没放）→ 提到判断之前，触发 **0→15 次**。⚠️ **最终根因是麦克风信噪比**：满量程仅 **1.66%**（正常几十个百分点），扬声器外放录进来同样只有 2.0% ⇒ 系统输入增益过低，**等比增益改变不了峰谷比**（1.84x→1.83x，仍卡在 2.5x 门槛）⇒ 需用户调系统音量/换近距离麦克风。**判据：能打开≠有信号，判活性唯一标准是"数据在变"。** 明细见 `sprite-history.md` |
>
