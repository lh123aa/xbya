...
│   ├── check_repo_hygiene.py        # G13：版本控制/忽略规则/密钥/模板落后（17 项）
...
| 两个 EventBus 并存（D14） | ⬜ 排期 P4-B1（先想清适配层边界） |
| ~~短句确认语在 ASR 下不稳（「算了」2/4）~~ | ✅ **已修（P4-B5）**：`--short` 模式把结果分三类报（正识/空输出/听错），基线 18/24；加解码偏置后 **24/24**（已落 `plugins.asr.params.initial_prompt`）。★ **顺带抓到并修掉一个危险得多的缺陷**：`_matches_any` 是朴素子串匹配，而确认语含单字「好」、取消语含单字「否」⇒ 待确认时**一句「你好呀」会被判成 `confirm`，从而放过待确认的删除**（实测 5 条错判，属 R1 最高风险）。修法写进契约：**确认从严、取消从宽**（四类出错方向代价不对称） |
| TTS 替身偶发劣化 | ⚠️ 已隔离（固定测激），**成因未定位**（那一版音频没留档）；P4-C1 |
| ASR `medium` 档 / sentence-transformers 真模型 / 百度谷歌反爬 / `web_open` 真开 | ⚠️ 外部条件不具备，逐条写明缺什么；P4-C2~C5 |
| D3 多用户隔离 | ⬜ 不做（无产品需求） |
